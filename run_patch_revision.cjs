...
// The replacement HTML and logic
const revisionUi = `
<style>
.revision-container { max-width: 800px; margin: 0 auto; padding: 20px; }
.card { background: white; border-radius: 12px; padding: 24px; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1); margin-bottom: 24px; }
textarea { width: 100%; min-height: 150px; padding: 12px; border: 1px solid #e5e7eb; border-radius: 8px; margin-bottom: 16px; font-family: inherit; font-size: 14px; resize: vertical; }
...
.summary-box li { margin-bottom: 8px; line-height: 1.5; color: #4b5563; }
...
<div class="revision-container">
    <div class="header" style="margin-bottom: 24px;">
        <h1 style="font-size: 24px; font-weight: 700; color: #111827; margin-bottom: 8px;">Quick Revision & Practice</h1>
        <p style="color: #6b7280;">Paste your notes or study material below to generate concise revision points and a personalized quiz.</p>
    </div>
    ...
    <div class="card" id="inputSection">
        ...
        <div id="loadingSummary" class="hidden" style="text-align: center; margin-top: 16px; color: #6b7280;">
            <div class="loader"></div> <span style="margin-left:8px;">Extracting key points...</span>
        </div>
    </div>
    ...
    <div class="card hidden" id="summarySection">
        ...
        <div id="loadingQuiz" class="hidden" style="text-align: center; margin-top: 16px; color: #6b7280;">
            <div class="loader"></div> <span style="margin-left:8px;">Creating personalized questions from your notes...</span>
        </div>
    </div>
...
<script>
// We use an IIFE to not pollute global scope, and listen to DOMContentLoaded
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Markdown render utility
    function renderMarkdown(text) {
        // Very basic markdown list renderer 
        let htmlResponse = marked ? marked.parse(text) : text;
        return htmlResponse;
    }

    // Attempt to load marked.js if not present
    ...
    // Initialize Lucide icons specific to our custom buttons if lucide is available
    ...
    btnGenerateSummary.addEventListener('click', async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                summaryContent.innerHTML = typeof marked !== 'undefined' ? marked.parse(generatedNotes) : '<pre>' + generatedNotes + '</pre>';
    ...
    btnSubmitQuiz.addEventListener('click', () => {
        ...
        generatedQuestions.forEach((q, qIndex) => {
            ...
            const correctOptIndex = q.answer;
...
const mainStart = content.indexOf('<main class="main-content">') + '<main class="main-content">'.length;
const mainEnd = content.indexOf('</main>', mainStart);

if(mainStart > -1 && mainEnd > -1) {
    let newContent = content.substring(0, mainStart) + '\n' + revisionUi + '\n' + content.substring(mainEnd);
    
    // Also update the title
    newContent = newContent.replace('<title>Smart Quiz', '<title>Quick Revision');
    
    // De-activate the nav item and add the new one
    newContent = newContent.replace(/<a class="nav-item active".*?href="quiz.html"/, '<a class="nav-item" data-page="quiz.html" href="quiz.html"');
    
    // Find where the sidebar nav items end to insert our new one
    const revisionNavItem = `
            <a class="nav-item active" data-page="revision.html" href="revision.html">
...
`;
    // Replace the quiz link with quiz link AND revision link
    const quizLinkPattern = /<a class="nav-item" data-page="quiz.html" href="quiz.html">[\s\S]*?<\/a>/;
    const match = newContent.match(quizLinkPattern);
    if(match) {
        newContent = newContent.replace(match[0], match[0] + revisionNavItem);
    }
    
    fs.writeFileSync(targetFile, newContent, 'utf8');
    console.log("Success patched main content of revision.html");
} else {
    console.log("Could not patch UI");
}
